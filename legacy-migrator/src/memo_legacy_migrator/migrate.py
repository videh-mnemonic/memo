...
def legacy_sources(
    paths: StoragePaths | None = None, *, legacy_dir: Path | None = None
) -> list[LegacySource]:
    paths = paths or StoragePaths.discover()
    result: list[LegacySource] = []

    def add_recording_dirs(directory: Path, label_prefix: str) -> None:
        if directory.is_dir():
            for child in sorted(directory.iterdir()):
                if (child / "meta.json").is_file():
                    result.append(LegacySource(f"{label_prefix}:{child.name}", child))

    def add_archives(directory: Path, label_prefix: str) -> None:
        if directory.is_dir():
            for archive in sorted(directory.glob("*/*.tar.gz")):
                result.append(
                    LegacySource(
                        f"{label_prefix}:{archive.parent.name}/{archive.name}",
                        archive,
                        True,
                    )
                )
    ...
    if legacy_dir is not None:
        root = legacy_dir.expanduser().resolve()
        if not root.is_dir():
            raise NotADirectoryError(f"legacy recordings directory not found: {root}")
        add_recording_dirs(root, "legacy-dir")
        add_recording_dirs(root / "scratch", "legacy-dir:scratch")
        add_archives(root / "archive", "legacy-dir:archive")
        return result

    add_recording_dirs(paths.home / "scratch", "scratch")
    add_archives(paths.archive, "archive")
    return result
...
def migrate_legacy(
    ...
    *,
    ...
    sources = legacy_sources(paths, legacy_dir=legacy_dir)
