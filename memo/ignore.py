...
class IgnorePolicy:
    """Git-compatible ignore rules with nested files and Memo self-exclusion."""
    ...
    def _self_excluded_roots(self) -> list[Path]:
        if self.paths is None:
            return []
        candidates = [self.paths.runtime, self.paths.archive, self.paths.scratch]
    ...
    def _specs(self, parent: Path) -> list[tuple[Path, str, GitIgnoreSpec]]:
        ...
        for directory in directories:
            for filename in (".gitignore", ".memoignore"):
    ...
    def decision(self, path: Path, is_dir: bool = False) -> IgnoreDecision:
        ...
        ignored = False
        source = None
        for base_path, filename, spec in self._specs(path.parent):
            try:
