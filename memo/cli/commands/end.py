...
from .common import format_size, session_size
...
def run(args: Any) -> int:
    ...
    size = format_size(session_size(session_path))
    print(f"{action}: {response['session_id']} step={response['step']} size={size}")
    if response.get("cloud") == "pushed":
