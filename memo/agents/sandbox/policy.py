...
def resolve_policy(
    ...
    *,
    ...
    for value in config.system_read_only:
        path = expand_path(value, home).resolve(strict=True)
        mounts.append(Mount(path, path, "read", "system"))
    for value in config.system_read_only_if_present:
        path = expand_path(value, home).resolve(strict=False)
        if path.exists():
            mounts.append(Mount(path.resolve(strict=True), path, "read", "system"))
        else:
            missing.append(path)
    mounts.append(Mount(root, root, "read-write", "recording-root"))
    mounts.extend(_git_mounts(root))
    for value in config.home_read_write_if_present:
