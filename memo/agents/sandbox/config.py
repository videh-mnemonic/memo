...
class SandboxConfig:
    network: bool
    gpu: bool
    home_read_only_if_present: tuple[str, ...]
...
def parse_config(data: bytes, source: str = "sandbox policy") -> SandboxConfig:
    ...
    if set(home) - {"read_only_if_present", "read_write_if_present"}:
    ...
    return SandboxConfig(
        ...
        gpu=value["gpu"],
        home_read_only_if_present=_strings(
            home.get("read_only_if_present", []), "home.read_only_if_present"
        ),
...
def render_config(config: SandboxConfig) -> bytes:
    lines = [
        ...
        + ", ".join(_quote(item) for item in config.home_read_only_if_present)
        + "]",
