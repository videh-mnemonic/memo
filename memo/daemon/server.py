...
class MemoDaemon:
    ...
    def _end(self, payload: dict[str, Any]) -> dict[str, Any]:
        self._expire_stale_attachments()
        s3_config = self._s3_config(payload, required=True)
    ...
    def _push(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        from ..transport.remote_sessions import _completion_key, _list_completions, _session_base

        config = self._s3_config(payload, required=True)
        assert config is not None
        from ..transport.remote_sessions import _store

        remote = _store(config, None)
